...
"http://guide.wisc.edu/courses/zoology/",
    ...
    rules = (
            ...
            Rule( LinkExtractor( allow = ( r'http://guide.wisc.edu/courses/' )),
    ...
    def parse_httpbin( self, response ):
        ...
        get_courses = response.css('span.courseblockcode::text').extract() 
        
        prerequisites  = response.css('p.courseblockextra.noindent.clearfix > span.cbextra-data > .bubblelink::text').extract()

        course_size = len(get_courses)
        prereq_size = len(prerequisites)
        ...
        while i < course_size:
            ...
            course = response.css('span.courseblockcode::text').extract()[ i ]
            #course = response.css('span.courseblockcode::text').extract_first()
            #course = data.css('span.courseblockcode::text').extract()
            #course = data.css('span.courseblockcode::text').re()


            title = response.css('div.sc_sccoursedescs > div.courseblock > p.courseblocktitle > strong::text').extract()[ i ]
            #title = response.css('div.sc_sccoursedescs > div.courseblock > p.courseblocktitle > strong::text').extract_first()
            #title = data.css('div.sc_sccoursedescs > div.courseblock > p.courseblocktitle > strong::text').extract()
            #title = data.css('div.sc_sccoursedescs > div.courseblock > p.courseblocktitle > strong::text').extract_first()

            unit = response.css('.courseblockcredits::text').extract()[ i ]
            #unit = response.css('.courseblockcredits::text').extract_first()
            #unit = data.css('.courseblockcredits::text').extract()
            #unit = data.css('.courseblockcredits::text').re()

            description =  response.css('.courseblockdesc::text').extract()[ i ]
            #description =  response.css('.courseblockdesc::text').extract_first()
            #description = data.css('.courseblockdesc::text').extract()
            #description = data.css('courseblockdesc::text').re() 

            #prerequisites =  response.css('p.courseblockextra.noindent.clearfix > span.cbextra-data > .bubblelink::text').extract()[ i ]
            #prerequisites = response.css('p.courseblockextra.noindent.clearfix > span.cbextra-data > .bubblelink::text').extract_first()
            #prerequisites = data.css('p.courseblockextra.noindent.clearfix > span.cbextra-data > .bubblelink::text').extract()
            #prerequisites = data.css('p.courseblockextra.noindent.clearfix > span.cbextra-data > .bubblelin::text').re() 
            #prerequisites = response.css('p.courseblockextra.noindent.clearfix > span.cbextra-data > .bubblelink::text').extract( Default = 'out_of_bound')[ i ]
        
            #try: 
                #prerequisites  = response.css('p.courseblockextra.noindent.clearfix > span.cbextra-data > .bubblelink::text').re

                #prerequisites = response.xpath('//div[@class="cb-extras"]/p/span[@class="cbextra-data"]/a[@class="bubblelink code"]/text()').extract()[ i ]

            #except AttributeError:
                  
            #prerequisites = response.css('div > #left-col > #content > #textcontainer > div  > div > div > p > span.cbextra-data::text').extract()[ i ]

            #prerequisites = response.xpath('//div[@class="cb-extras"]/p/span[@class="cbextra-data"]/text()').extract()[ i ] 
            ...
            item['prerequisites'] = prerequisites[ i ] 
