...
"http://guide.wisc.edu/courses/zoology/",
    ...
    def parse_httpbin( self, response ):
        ...
        data_set =  response.css('main > div#column-wrapper > div.wrap.clearfix')
        
        items = []
    
        for data in response.css('main'): 

            #item = UwMadisonItem()

            yield {

                    'course' : data.css('span.courseblockcode::text').extract_first(),
                    'title' : data.css('div.sc_sccoursedescs > div.courseblock > p.courseblocktitle > strong ::text').extract_first(),
                    'unit': data.css('.courseblockcredits ::text').extract_first(),
                    'description': data.css('.courseblockdesc ::text').extract_first(),
                    'prerequisites': data.css('p.courseblockextra.noindent.clearfix > span.cbextra-data > .bubblelink ::text').extract_first()
