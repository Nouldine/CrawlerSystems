...
"https://www.wisconsin.edu/transfer/wizards/?tis=edu.uwsa.tis.Wizards.JSP%252FdepartmentList.do%253Bjsessionid%253D1F66711060E286D89B93ED5E4F168BF8&reqType=C&reqType=C&tispage=1&fromInstitutionId=4599&toInstitutionId=4599&submit=Next+Step",
    ...
    def parse_httpin( self, response ):
        ...
        departmemt_name = response.xpath('//select[@name="departmentId"]/option/text()').extract()
        from_campus_id  = response.xpath('//input[@name="fromInstitutionId"]/@value').extract()
        
        to_campus_id    = response.xpath('//select[@name="toInstitutionId"]/option/@value').extract_first()

        from_institution_final  = []
        institution_name = []


        for depart in departmemt_name: 

            depart_1 = "".join([ s for s in depart.splitlines(True) if s.rstrip()])
    

            depart_2 = depart_1.replace('\n','')

            depart_3 = depart_2.replace('            ', '')
            
            institution_name.append(depart_2)
        ...
        while track_index < vec_size:
            ...
            items = AutomateLinksItem()
            
            items['department_id']      = ( department_id[ track_index ].rstrip() ).strip()
            items['department_name']    = ( institution_name[ track_index ].rstrip() ).strip()
            items['from_campus_id']     = from_campus_id 
            items['to_campus_id']       = to_campus_id
